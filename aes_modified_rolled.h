...
static inline void AesEncryptExpand(uint32_t *roundkey, const uint8_t *key) {
	...
	for (uint8_t index = 4; index < 52; index += 4) {
		roundkey[index] = roundkey[index - 4] ^ SubWord(roundkey[index - 1]) ^ rcon[index / 4 - 1];
...
static inline void AesEncrypt(const uint32_t *roundkey, const uint8_t plaintext[AES_BLOCK_SIZE], uint8_t ciphertext[AES_BLOCK_SIZE]) {
    ...
    // 12 rounds of AES encryption
    for (int round = 0; round < 11; ++round) {
        // SubBytes and ShiftRows
        t0 = Te0[s0 >> 24] ^ Te1[(s1 >> 16) & 0xff] ^ Te2[(s2 >> 8) & 0xff] ^ Te3[s3 & 0xff] ^ roundkey[4 * (round + 1)];
        t1 = Te0[s1 >> 24] ^ Te1[(s2 >> 16) & 0xff] ^ Te2[(s3 >> 8) & 0xff] ^ Te3[s0 & 0xff] ^ roundkey[4 * (round + 1) + 1];
        t2 = Te0[s2 >> 24] ^ Te1[(s3 >> 16) & 0xff] ^ Te2[(s0 >> 8) & 0xff] ^ Te3[s1 & 0xff] ^ roundkey[4 * (round + 1) + 2];
        t3 = Te0[s3 >> 24] ^ Te1[(s0 >> 16) & 0xff] ^ Te2[(s1 >> 8) & 0xff] ^ Te3[s2 & 0xff] ^ roundkey[4 * (round + 1) + 3];
    ...
    // Final round: AddRoundKey and mapping to ciphertext
...
static inline void AesDecrypt(const uint32_t *roundkey, const uint8_t ciphertext[AES_BLOCK_SIZE], uint8_t plaintext[AES_BLOCK_SIZE]) {
    ...
    for (int round = 0; round < 11; ++round) {
        // SubBytes and ShiftRows
        t0 = Td0[s0 >> 24] ^ Td1[(s3 >> 16) & 0xff] ^ Td2[(s2 >>  8) & 0xff] ^ Td3[s1 & 0xff] ^ roundkey[4 * (round + 1)];
        t1 = Td0[s1 >> 24] ^ Td1[(s0 >> 16) & 0xff] ^ Td2[(s3 >>  8) & 0xff] ^ Td3[s2 & 0xff] ^ roundkey[4 * (round + 1) + 1];
        t2 = Td0[s2 >> 24] ^ Td1[(s1 >> 16) & 0xff] ^ Td2[(s0 >>  8) & 0xff] ^ Td3[s3 & 0xff] ^ roundkey[4 * (round + 1) + 2];
        t3 = Td0[s3 >> 24] ^ Td1[(s2 >> 16) & 0xff] ^ Td2[(s1 >>  8) & 0xff] ^ Td3[s0 & 0xff] ^ roundkey[4 * (round + 1) + 3];
