...
#define GETU32(input_data) (((uint32_t)(input_data)[0] << 24) ^ \
						((uint32_t)(input_data)[1] << 16) ^ \
						((uint32_t)(input_data)[2] <<  8) ^ \
						((uint32_t)(input_data)[3] <<  0))
...
static inline bool AES_EncryptInit(AES_CTX *ctx, const uint8_t *key, const uint8_t *iv) {
	if (!ctx || !key || !iv) {
		return false;
	}
	
	memcpy(ctx->iv, iv, 16);
	memcpy(ctx->block, iv, 16);
...
static inline uint8_t AES_DecryptInit(AES_CTX *ctx, const uint8_t *key, const uint8_t *iv) {
	if (!AES_EncryptInit(ctx, key, iv)) {
		return false;
	}
...
static inline uint32_t AES_Encrypt(AES_CTX *ctx, const uint8_t *input_data, uint32_t input_size, uint8_t *output_data) {
	uint32_t s0, s1, s2, s3, t0, t1, t2, t3, offset;
	
	for (offset = 0; offset + AES_BLOCK_SIZE <= input_size; offset += AES_BLOCK_SIZE) {
		
		for (uint8_t index = 0; index < AES_BLOCK_SIZE; index++) {
			output_data[offset + index] = input_data[offset + index] ^ ctx->block[index];
		}
		
		s0 = GETU32(output_data + offset +  0) ^ ctx->roundkey[0];
		s1 = GETU32(output_data + offset +  4) ^ ctx->roundkey[1];
		s2 = GETU32(output_data + offset +  8) ^ ctx->roundkey[2];
		s3 = GETU32(output_data + offset + 12) ^ ctx->roundkey[3];
		...
		PUTU32(output_data + offset + 4, s1);
		...
		PUTU32(output_data + offset + 8, s2);
		...
		PUTU32(output_data + offset + 12, s3);
		
		memcpy(ctx->block, output_data, 16);
	}
	return offset;
}

static inline uint32_t AES_Decrypt(AES_CTX *ctx, const uint8_t *input_data, uint32_t input_size, uint8_t *output_data) {
	uint32_t s0, s1, s2, s3, t0, t1, t2, t3, offset;
	memcpy(ctx->iv, input_data, 16);
	
	for (offset = 0; offset + AES_BLOCK_SIZE <= input_size; offset += AES_BLOCK_SIZE) {
		s0 = GETU32(input_data + offset) ^ ctx->roundkey[0];
		s1 = GETU32(input_data + offset + 4) ^ ctx->roundkey[1];
		s2 = GETU32(input_data + offset + 8) ^ ctx->roundkey[2];
		s3 = GETU32(input_data + offset + 12) ^ ctx->roundkey[3];
		...
		for (uint8_t index = 0; index < AES_BLOCK_SIZE; index++) {
			output_data[offset + index] ^= ctx->block[index];
		}
		
		memcpy(ctx->block, ctx->iv, 16);
